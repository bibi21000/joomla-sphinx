...
function SphinxdocBuildRoute(&$query)
{
	...
	$menu		= $app->getMenu();
	$params		= JComponentHelper::getParams('com_sphinxdoc');
...
function SphinxdocParseRoute($segments)
{
	...
	if ($count == 1) {
		...
		if ($category && $category->alias == $alias) {
			...
			$query = 'SELECT alias, catid FROM #__sphinxdoc WHERE id = '.(int)$id;
			$db->setQuery($query);
	...
	if (!$category) {
		JError::raiseError(404, JText::_('COM_SPHINXDOC_ERROR_PARENT_CATEGORY_NOT_FOUND'));
		return $vars;
	}
	...
	foreach($segments as $segment)
	{
		...
		if ($found == 0) {
			if ($advanced) {
				$db = JFactory::getDBO();
				$query = 'SELECT id FROM #__sphinxdoc WHERE catid = '.$vars['catid'].' AND alias = '.$db->Quote($segment);
				$db->setQuery($query);
				$cid = $db->loadResult();
			} else {
				$cid = $segment;
